...
//判断像素中心点是否在三角形内部
//使用叉乘
//第三个数是_v指向Vector3f类型的指针，默认传入了三角形的三个顶点
...
void rst::rasterizer::rasterize_triangle(const Triangle& t) {
    ...
    // 创建bounding box 寻找三角形最小点和最大点(均为整型)
	...
	for (int x = nMin_x; x <= nMax_x; x++)
	{
		...
		{
            //判断像素点是否在三角形中, 像素坐标需偏移0.5
			if (insideTriangle((float)x+0.5, (float)y+0.5, t.v))
			{
                //根据已有代码获取深度值， 即z_interpolated
                ...
                //如果当前位置深度比depth_buf更小，则更新颜色值并保存深度值
